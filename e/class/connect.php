...
//网游结合项页码
function kaifameigamepage_xz59($num,$line,$page_line,$start,$page,$search2){
	global $fun_r;
	$num=(int)$num;
	$line=(int)$line;
	$page_line=(int)$page_line;
	...
	if($page<>0)
	{
		$toppage='<a href="'.$url.$search.'0.html"><span>'.$fun_r['startpage'].'</span></a>';
		$pagepr=$page-1;
		$prepage='<a href="'.$url.$search.$pagepr.'.html"><span>'.$fun_r['pripage'].'</span></a>';
	}
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='<a href="'.$url.$search.$pagenex.'.html"><span>'.$fun_r['nextpage'].'</span></a>';
		$lastpage='<a href="'.$url.$search.($totalpage-1).'.html"><span>'.$fun_r['lastpage'].'</span></a>';
	}
	$starti=$page-$snum<0?0:$page-$snum;
	$no=0;
	for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
	{
		...
		{
			$is_1='<span><a href="'.$url.$search.$i.'.html">';
			$is_2="</a></span>";
		}
		$pagenum=$i+1;
		$returnstr.=$is_1.$pagenum.$is_2;
...
//网游结合项页码
function kaifameigamepage($num,$line,$page_line,$start,$page,$search2){
    global $fun_r;
    $num=(int)$num;
    $line=(int)$line;
    $page_line=(int)$page_line;
    ...
    if($page<>0)
    {
        $toppage='<a href="'.$url.$search.'0.html" class="fristP">'.$fun_r['startpage'].'</a>';
        $pagepr=$page-1;
        $prepage='<a href="'.$url.$search.$pagepr.'.html" class="fristP">'.$fun_r['pripage'].'</a>';
    }else{
        $toppage='<a href="'.$url.$search.'0.html" class="fristP">'.$fun_r['startpage'].'</a>';
        $prepage='<a href="'.$url.$search.'0.html" class="fristP">'.$fun_r['pripage'].'</a>';
    }
    //下一页
    if($page!=$totalpage-1)
    {
        $pagenex=$page+1;
        $nextpage='<a href="'.$url.$search.$pagenex.'.html" class="lastP">'.$fun_r['nextpage'].'</a>';
        $lastpage='<a href="'.$url.$search.($totalpage-1).'.html" class="lastP">'.$fun_r['lastpage'].'</a>';
    }else{
        $nextpage='<a href="javascript:;" class="lastP">'.$fun_r['nextpage'].'</a>';
        $lastpage='<a href="javascript:;" class="lastP">'.$fun_r['lastpage'].'</a>';
    }
    $starti=$page-$snum<0?0:$page-$snum;
    $no=0;
    for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
    {
        ...
        if($page==$i)
        {
            $is_1="<a class=\"page_num page_no\">";
            $is_2="</a>";
        }
        else
        {
            $is_1='<a href="'.$url.$search.$i.'.html" class="page_num">';
            $is_2="</a>";
        }
        $pagenum=$i+1;
        $returnstr.=$is_1.$pagenum.$is_2;
...
//软件结合项页码
function kaifameisoftpage($num,$line,$page_line,$start,$page,$search2){
	global $fun_r;
	$num=(int)$num;
	$line=(int)$line;
	$page_line=(int)$page_line;
...
function InfoUsePage($num,$line,$page_line,$start,$page,$search,$add){
	...
	if($page<>0)
	{
		$toppage='<a href="'.eReturnRewritePageLink($add,0).'"><span>'.$fun_r['startpage'].'</span></a>';
		$pagepr=$page-1;
		$prepage='<a href="'.eReturnRewritePageLink($add,$pagepr).'"><span>'.$fun_r['pripage'].'</span></a>';
	}
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='<a href="'.eReturnRewritePageLink($add,$pagenex).'"><span>'.$fun_r['nextpage'].'</span></a>';
		$lastpage='<a href="'.eReturnRewritePageLink($add,$totalpage-1).'"><span>'.$fun_r['lastpage'].'</span></a>';
	}
	$starti=$page-$snum<0?0:$page-$snum;
	$no=0;
	for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
	{
		...
		{
			$is_1='<span><a href="'.eReturnRewritePageLink($add,$i).'">';
			$is_2="</a></span>";
		}
		$pagenum=$i+1;
		$returnstr.=$is_1.$pagenum.$is_2;
