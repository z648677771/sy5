...
//列表模板之列表式分页
function sys_ShowListMorePage_index($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	$firststr='<span class="a1 disabled">共'.$num.'页</span>';
	//上一页
	if($page<>1)
	{
...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
    ...
    if($page<>1)
    {
        $toppage='<a href="'.$dolink.'" class="fristP">'.$fun_r['startpage'].'</a>';
        $pagepr=$page-1;
        if($pagepr==1)
        {
            $prido=$add['dofile'].$type;
        ...
        $prepage='<a href="'.$dolink.'" class="fristP">'.$fun_r['pripage'].'</a>';
    }else{
        $toppage='<a href="'.$dolink.'" class="fristP">'.$fun_r['startpage'].'</a>';
        $prepage='<a href="'.$dolink.'" class="fristP">'.$fun_r['pripage'].'</a>';
    }
    ...
    if($page!=$totalpage)
    {
        ...
        $nextpagelink=$repagenum&&$repagenum<$pagenex?eReturnRewritePageLink2($add,$pagenex):$dolink.$add['dofile'].'_'.$pagenex.$type;
        $lastpagelink=$repagenum&&$repagenum<$totalpage?eReturnRewritePageLink2($add,$totalpage):$dolink.$add['dofile'].'_'.$totalpage.$type;
        $nextpage='<a href="'.$nextpagelink.'" class="lastP"><span>'.$fun_r['nextpage'].'</span></a>';
        $lastpage='<a href="'.$lastpagelink.'" class="lastP"><span>'.$fun_r['lastpage'].'</span></a>';
    }else{
        $nextpage='<a href="javascript:;" class="lastP"><span>'.$fun_r['nextpage'].'</span></a>';
        $lastpage='<a href="javascript:;" class="lastP"><span>'.$fun_r['lastpage'].'</span></a>';
    }
    $starti=$page-$snum<1?1:$page-$snum;
    $no=0;
    for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
    {
        ...
        elseif($i==1)
        {
            $is_1='<a href="'.$dolink.$add['dofile'].$type.'" class="page_num">';
            $is_2="</a>";
        }
        else
        {
            $thispagelink=$repagenum&&$repagenum<$i?eReturnRewritePageLink2($add,$i):$dolink.$add['dofile'].'_'.$i.$type;
            $is_1='<a href="'.$thispagelink.'" class="page_num">';
            $is_2="</a>";
        }
        $returnstr.=$is_1.$i.$is_2;
    }
